...
class SSEDataset(Dataset):
    def __init__(self, real_profile_data_path, sequence_data_path_prefix, blosum_path_prefix, att_gt_path, label_data_path_prefix, mode='all',
                 enable_downsample=True, config=config_sse):
        # mode: all low high
        self.mode = mode
        self.config = config

        self.real_profile_file_list = glob.glob(real_profile_data_path)

        # file_list = glob.glob(real_profile_data_path)
        # self.real_profile_file_list = []
        # fake_file_list = list(map(lambda x:x.replace('uniref90/uniref90_bc40_real_nogap', 'fake_profile_bc40_FAIR'), file_list))
        # for i in range(len(fake_file_list)):
        #     if os.path.exists(fake_file_list[i]):
        #         self.real_profile_file_list.append(file_list[i])

        self.post_fix = '.npy'
        self.blosum_path_prefix = blosum_path_prefix
        self.att_gt_path = att_gt_path

        if mode == 'high':
            self.real_profile_file_list = list(
                filter(lambda x: x.split('/')[-1].split(self.post_fix)[0] in self.config.high_quality_list,
                       self.real_profile_file_list))
            print('high msa number:', len(self.real_profile_file_list))
        elif mode == 'low':
            self.real_profile_file_list = list(
                filter(lambda x: x.split('/')[-1].split(self.post_fix)[0] in self.config.low_quality_list,
                       self.real_profile_file_list))
            print('low msa number:', len(self.real_profile_file_list))
        ...
        self.att_gts = np.load(self.att_gt_path, allow_pickle=True).item()
    ...
    def __getitem__(self, index):
        real_profile_file = self.real_profile_file_list[index]
        filename = real_profile_file.split('/')[-1].split(self.post_fix)[0]

        sequence_file = self.sequence_data_path + filename + '.fasta'
        label_file = self.label_data_path + filename + '.label'

        # load data
        fake_profile_file = self.config.profile_fake_data_path_prefix.replace('train', 'valid') +filename+'.npy'
        ...
        # blosum_array = np.load(self.blosum_path_prefix + filename + '.npy')

        att_gt = np.zeros((20,20))


        return filename, token_ids, fake_profile_array, real_profile_array, label, low_profile_array, att_gt

    # one epoch 6125 samples
    def __len__(self):
        return len(self.real_profile_file_list)

    def collate_fn(self, batch):
        # gap model
        filename, input_ids, fake_profile_array, real_profile_array, label, low_profile, att_gt = tuple(zip(*batch))
        ...
        att_gt = torch.from_numpy(pad_sequences(att_gt))
        return {'filename': filename[0], 'sequence': input_ids, 'fake_profile': fake_profile_array, 'real_profile': real_profile_array, 'label': label,
                'low_profile': low_profile, 'att_gt': att_gt}
