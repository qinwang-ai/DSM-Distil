...
class SSEDataset(Dataset):
    def __init__(self, fake_profile_data_path, sequence_data_path_prefix, blosum_path_prefix, att_gt_path,
                 real_profile_data_prefix,
                 label_data_path_prefix, mode='all',
        ...
        self.real_profile_data_prefix = real_profile_data_prefix
    ...
    def get_low_profile(self, msa_list):
        ...
        return profile, num_msa_for_profile
    ...
    def __getitem__(self, index):
        ...
        # load data
        real_profile_file = self.real_profile_data_prefix + filename + '.npy'
        ...
        if self.enable_downsample:
            if self.mode == 'high' or (self.mode == 'all' and filename in self.config.high_quality_list):
                ...
                low_profile_array, msa_c1 = self.get_low_profile(msa_strs)
            else:
        ...
        att_gt = self.att_gts[filename]
        # att_gt = np.zeros((len(token_ids), 2))
