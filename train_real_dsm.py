...
def try_get_pretrained(ssenet, scratch=False):
    ssenet_path = config.pretrain_path + 'ssenet_real_dsm.pth'
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if not is_test:
            ...
            for batch in tqdm(sse_loader):
                ...
                optimizer.zero_grad()
                pred, _ = ssenet(sequence, real_profile)  # 16 x 44 x 3
        ...
        for batch in tqdm(val_loader):
            ...
            pred, _ = ssenet(sequence, real_profile)
            pred_no_pad = pred[label != -1, :]
            label_no_pad = label[label != -1]
            errCE = ce_loss_func(pred_no_pad, label_no_pad)
        ...
        if is_inference:
            ...
            # f = open('logs/real_dsm_acc3_bc40.txt', 'w')
            f = open('logs/real_dsm_acc3_6125.txt', 'w')
...
if __name__ == '__main__':
    ...
    if is_inference:
        sse_val_dataset = SSEDataset(config.profile_fake_data_path.replace('train', 'valid'),
