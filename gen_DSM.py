...
def parse_batch(batch):
    ...
    return sequence, profile, label, msa_c, filename

def save_dsm(DSM, filename):
    path = config.DSM_real_path_prefix + 'train/' + filename + '.npy'
    np.save(path, DSM.cpu().squeeze().detach().numpy())
...
def train(sse_loader, val_loader, ssenet, is_test=False):
    ...
    for batch in tqdm(val_loader):
        sequence, profile, label, msa_c, filename = parse_batch(batch)

        pred, DSM, N = ssenet(profile, msa_c)
        ...
        save_dsm(DSM, filename)
        cls = msa_c.squeeze().argmax(dim=-1).item()
        if cls not in dsm_dict:
            dsm_dict[cls] = []
        dsm_dict[cls].append(N.item())

    # statistic
    summary_np = np.array(summary)
    _summary = summary_np[:, 1:].astype(np.float).mean(axis=0)
    print('[EVAL]', 'CE_loss: %0.3f, curr_acc: %0.3f' % (_summary[0], _summary[1]))

    lastv = 0
    for k,v in dsm_dict.copy().items():
        if len(v) < 3:
            dsm_dict[k] = lastv
        else:
            dsm_dict[k] = np.array(v).mean()
        lastv = dsm_dict[k]

    d = sorted(dsm_dict.items())
...
if __name__ == '__main__':
    ...
    stretch_path = config.pretrain_path + 'stretchnet.pth'
    torch.save(ssenet.stretchNet.state_dict(), stretch_path)
