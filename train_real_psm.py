    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if not is_test:
            ...
            for batch in tqdm(sse_loader):
                sequence, psm, label, _ = parse_batch(batch)

                optimizer.zero_grad()
                pred = ssenet(sequence, psm.softmax(dim=-1))  # 16 x 44 x 3
        ...
        for batch in tqdm(val_loader):
            ...
            pred = ssenet(sequence, psm.softmax(dim=-1))
            pred_no_pad = pred[label != -1, :]
            label_no_pad = label[label != -1]
            errCE = ce_loss_func(pred_no_pad, label_no_pad)
        ...
        _std = summary_np[:, 1:].astype(np.float).std(axis=0)

        if not is_test and _summary[-1] > prev_best:
            prev_best = _summary[-1]
            save_model()

        print('[EVAL]', 'CE_loss: %0.3f, curr_acc: %0.3f, best_acc:%0.3f, std:%0.3f' % (_summary[0], _summary[1], prev_best, _std[-1]))
        if is_test:
            # summary = sorted(summary, key=lambda x:x[-1])
            # f = open('./logs/real_psm3_bc40.txt', 'w')
            # f = open('./logs/real_psm3_6125.txt', 'w')
...
if __name__ == '__main__':
    ...
    sse_dataset = SSEDataset(config.psm_real_data_path,
                             ...
                             config.label_data_path_prefix, enable_downsample=False, config=config)
    ...
    sse_val_dataset = SSEDataset(config_tmp.psm_real_data_path.replace('train', 'valid'),
                                 ...
                                 enable_downsample=False, mode='low', config=config_tmp)
    ...
    # BEST ACC with BC40 < 5: 0.727
    # BEST ACC with BC40 < 3: 0.717
