...
def try_get_pretrained_raw(ssenet_lowreal, ssenet_fake):
    ssenet_low_real_path = config.pretrain_path + 'ssenet_raw_lowprofile8.pth'
    ssenet_fake_path = config.pretrain_path + 'ssenet_raw_fakeprofile8.pth'
...
def try_get_pretrained(teacher, student, gradenet, scratch=False):
    teacher_path = '../module/bio_teacher8.pth'
    student_path = '../module/bioinfo_student8.pth'
    gradenet_path = '../module/bioinfo_gradenet8.pth'
    ...
    return teacher.cuda(), student.cuda(), gradenet.cuda()
...
def train(sse_loader, val_loader, teacher, student, gradenet, epochs=config.epochs):

    optimizer = torch.optim.Adam(list(student.parameters()) + list(gradenet.parameters()), lr=config.gen_lr, weight_decay=1e-8)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epochs != 1:
            ...
            for batch in tqdm(sse_loader):
                ...
                # GradeNet - data augmentation
                if random.random() < 0.5:
                    feats = torch.cat([low_feats, fake_feats], dim=-1)
                    errGrade, (u,v), grade_acc = get_grade_loss(sequence, feats, att_gt, gradenet, label)
                else:
                    feats = torch.cat([fake_feats, low_feats], dim=-1)
                    errGrade, (v,u), grade_acc = get_grade_loss(sequence, feats, 1-att_gt, gradenet, label)

                dsm = u * low_dsm + v * fake_dsm
                feats = u * low_feats + v * fake_feats

                # Obtain SSE CE loss
                errCE, acc, x_d = get_ce_loss(sequence, dsm, feats, label, student)
        ...
        for batch in tqdm(val_loader):
            ...
            # profile transform
            low_feats, fake_feats, low_dsm, fake_dsm = get_raw_features(sequence, real_profile, fake_profile)
            feats = torch.cat([low_feats, fake_feats], dim=-1)
            ...
            errCE_stu, acc, x_d = get_ce_loss(sequence, dsm, feats, label, student)
        ...
        if is_test:
            ...
            # f = open('logs/our_acc8_6125.txt', 'w')
            f = open('logs/our_acc8_bc40.txt', 'w')
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        student = Student(input_dim=config.embed_dim + config.profile_width + config.num_labels)
        ...
        teacher, student, gradenet = try_get_pretrained(teacher, student, gradenet, scratch=False)
        if is_test:
            train(sse_loader, sse_val_loader, teacher, student, gradenet, epochs=1)
        else:
            train(sse_loader, sse_val_loader, teacher, student, gradenet, epochs=300)
            break
