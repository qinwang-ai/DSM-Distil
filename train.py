...
def parse_batch(batch):
    ...
    real_profile = torch.tensor(batch['real_profile'], dtype=torch.float32).cuda()[:,:,:-1]
    fake_profile = torch.tensor(batch['fake_profile'], dtype=torch.float32).cuda()
    low_profile = torch.tensor(batch['low_profile'], dtype=torch.float32).cuda()[:,:,:-1]
...
def train(sse_loader, val_loader, teacher, student, gradenet, epochs=config.epochs):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if is_test:
            ...
            # f = open('logs/our_acc3_6125.txt', 'w')
