...
def get_ce_loss(sequence, psm, label):
    pred, _ = ssenet(sequence, psm)  # 16 x 44 x 3
...
def train_gen(sse_loader, val_loader, generator):
    optimizer_gen = torch.optim.Adam(generator.parameters(), lr=0.0001, weight_decay=1e-8)
...
def test_sse(val_loader, generator, ssenet, epochs):
    ...
    f = open('logs/bagging_acc3_bc40.txt', 'w')
    generator.eval()
    for epoch in range(1, epochs + 1):
        ...
        for batch in tqdm(val_loader):
            ...
            profile_fake = generator(sequence, profile_gt)
            ...
            profile_fake[profile_fake<profile_fake.mean()] = profile_fake.min()
            enhance_pro = profile_fake.softmax(dim=-1)

            np.save('/data/proli/raw_data/visualization/bagging_pssm/'+filename+'.npy', enhance_pro.cpu().squeeze().detach().numpy())
            print(enhance_pro.shape, 'saved..')
...
if __name__ == '__main__':
    ...
    if is_test:
        ...
        train_gen(sse_loader, sse_val_loader, generator)
