...
def try_get_pretrained(ssenet, generator, scratch=config.train_scratch):
    ssenet_path = '../module/ssenet_real_ref90_psm8.pth'
    generator_path = '../module/generator_bagging8.pth'

    ssenet.init_weights()
    import torch.nn.init as init
    from models.weight_initializer import Initializer
    Initializer.initialize(model=generator, initialization=init.xavier_uniform, gain=init.calculate_gain('relu'))
...
def train_gen(sse_loader, val_loader, generator, epochs=100):
    ...
    for epoch in range(1, epochs):
        ...
        if curr_acc > prev_best:
            prev_best = curr_acc
            save_model(generator, '../module/generator_bagging8.pth')
...
def test_sse(val_loader, generator, ssenet, epochs):
    ...
    f = open('logs/bagging_acc8_bc40.txt', 'w')
    generator.eval()
...
if __name__ == '__main__':
    ...
    ssenet, generator = try_get_pretrained(ssenet, generator, scratch=True)
