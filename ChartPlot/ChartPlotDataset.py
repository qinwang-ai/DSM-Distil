...
class ChartPlotDataset(Dataset):
    def __init__(self, real_psm_data_path, real_profile_path_prefix, sequence_data_path_prefix, label_data_path_prefix, mode='all', enable_downsample=True, config=configs.config_sse):
        # mode: all low high
        self.mode = mode
        ...
        if mode == 'high':
            ...
            self.psm_file_list = list(
                filter(lambda x: x.split('/')[-1].split('.npy')[0] not in config.high_quality_list,
                       self.psm_file_list))
        ...
        self.config = config
        super(ChartPlotDataset, self).__init__()
