...
from models.SSENetV3 import SSENet, StretchNetV2 as StretchNet
...
def get_dsm(profile):
    return stretchnet(profile)[0]

def try_get_pretrained(ssenet, stretchnet, scratch=False):
    ssenet_path = config.pretrain_path + 'ssenet_raw_fakeprofile8.pth'
    stretch_path = config.pretrain_path + 'ssenet_stretch8.pth'

    ssenet.init_weights()
    stretchnet.init_weights()
    stretchnet.load_state_dict(torch.load(stretch_path))

    if not scratch:
        if os.path.exists(ssenet_path):
            ssenet.load_state_dict(torch.load(ssenet_path))

    return ssenet.cuda(), stretchnet.cuda()
...
def train(sse_loader, val_loader, ssenet, is_test=False, is_inference=False):
    # optimizer = torch.optim.Adam(list(ssenet.parameters()) + list(stretchnet.parameters()), lr=config.gen_lr, weight_decay=1e-8)
    optimizer = torch.optim.Adam(list(ssenet.parameters()), lr=config.gen_lr, weight_decay=1e-8)
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if not is_test:
            ...
            for batch in tqdm(sse_loader):
                ...
                optimizer.zero_grad()
                # fake_profile = get_dsm(fake_profile)
                pred, _ = ssenet(sequence, fake_profile)  # 16 x 44 x 3
        ...
        for batch in tqdm(val_loader):
            ...
            # fake_profile = get_dsm(fake_profile)
            pred, _ = ssenet(sequence, fake_profile)
            pred_no_pad = pred[label != -1, :]
            label_no_pad = label[label != -1]
            errCE = ce_loss_func(pred_no_pad, label_no_pad)
        ...
        if is_inference:
            ...
            f = open('./single_net_err/fake_profile.txt', 'w')
...
def save_model():
    ...
    torch.save(stretchnet.state_dict(), stretch_path)
...
if __name__ == '__main__':
    ...
    stretchnet = StretchNet()

    # try load pretrained model
    ssenet, stretchnet = try_get_pretrained(ssenet, stretchnet, scratch=False)
